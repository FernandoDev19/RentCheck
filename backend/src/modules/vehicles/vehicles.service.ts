...
export class VehiclesService {
      ...
      qb.andWhere(
        new Brackets((orQb) => {
          orQb
            ...
            .orWhere('vehicle.insuredValue::text ILIKE :s', {
              s: `%${search}%`,
            });
        }),
      );
  ...
  async findAllAvailable(
    ...
    if ((user.role as RolesEnum) === RolesEnum.OWNER) {
      qb.where(
        'vehicle.renterId = :renterId & vehicle.status = :vehicleStatus',
      ...
      qb.where(
        'vehicle.branchId = :branchId & vehicle.status = :vehicleStatus',
  ...
  async rentVehicle(id: string, user: UserActiveInterface) {
    ...
    const isVehicleAvailable = vehicle.status === VehicleStatus.AVAILABLE;
