...
describe("Pending Feedbacks", () => {
    ...
    describe("Owner (Renter)", () => {
        ...
        it("should display the pending feedbacks page with correct structure", () => {
            cy.intercept("GET", "**/api/v1/rentals/pending-feedback*").as("getPendingFeedbacks");

            cy.visit("/owner/feedbacks");

            cy.contains("Feedbacks pendientes").should("be.visible");
            cy.contains("Rentas devueltas que aún no han sido calificadas").should("be.visible");
        ...
        it("should show the empty state message when no pending feedbacks", () => {
            ...
            cy.contains("No hay rentas pendientes de feedback 🎉").should("be.visible");
        });

        it("should search in the pending feedbacks list", () => {
            cy.intercept("GET", "**/api/v1/rentals/pending-feedback*").as("searchFeedbacks");
        ...
        it("should open the feedback form when clicking '✍️ Dar feedback'", () => {
            cy.intercept("GET", "**/api/v1/rentals/pending-feedback*").as("getPending");
        ...
        it("should show validation message when submitting feedback without scoring all fields", () => {
            cy.intercept("GET", "**/api/v1/rentals/pending-feedback*").as("getPending2");
        ...
        it("should activate critical flags and force all scores to 1", () => {
            cy.intercept("GET", "**/api/v1/rentals/pending-feedback*").as("getPending3");
            ...
            cy.wait("@getPending3").then(({ response }) => {
                ...
                cy.contains("Flag crítico activo — todas las calificaciones se fijan en 1").should("be.visible");
        ...
        it("should submit feedback successfully when all scores are provided", () => {
            cy.intercept("GET", "**/api/v1/rentals/pending-feedback*").as("getPending4");
            ...
            cy.wait("@getPending4").then(({ response }) => {
                ...
                const scoreFields = ["damageToCar", "unpaidFines", "arrears", "carAbuse", "badAttitude"];
                scoreFields.forEach((field) => {
                    cy.get(`input[name="score-${field}"][value="5"]`).check({ force: true });
                });

                // Optional comment
                cy.get('textarea[id="feedback-comments"]').type("Cypress automated feedback test.");
